  ...
  export default {
    ...
    methods: {
      ...
      edit(scope) {
        ...
        this.$router.push(`/admin_user/edit/${scope.row._id}`)
      },
      async remove(scope) {
        // 弹框
        this.$confirm(`此操作将永久删除"${scope.row.username}"管理员, 是否继续?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(async () => {
          const res =  await this.$http.delete(`rest/adminUsers/${scope.row._id}`)
          if (res.data.status) {
